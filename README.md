...
- 当前目录是从固定基线导出的干净、单提交公开源码快照候选；它不是最终 GitHub 发布。
...
- 确认仓库仍只有中性维护者身份创建的单个首提交，且没有 remotes 或 tags。
- 确认 `PUBLIC_FILES.txt` 与 `SHA256SUMS.txt` 和当前首提交一致。
...
项目级许可证待用户最终确认，当前仍在核对上游来源与权利链。当前仓库**不应被解释为已经采用 MIT 或其他项目级开源许可证**。
...
详见 [来源说明](SOURCE_PROVENANCE.md)、[第三方通知](THIRD_PARTY_NOTICES.md) 和 [商标说明](TRADEMARKS.md)。这些材料是事实记录，不构成法律意见。
