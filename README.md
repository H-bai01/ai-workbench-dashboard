...
- **通用文件管理**：自动识别已接入 AI 工具的工作目录，按工具和用途分类展示；支持查看、编辑、替换、删除、移动、重命名及调用系统程序打开文件。
- **通知中心**：集中展示运行与统计通知，已读历史仍可查看，错误通知可进入详情了解发生时间、影响范围和当前结果。
- **定时任务（Cron）中心**：查看和管理所有定时任务，显示发起方与任务去向，并提供更大的任务要求编辑区域。
- **OpenClaw 项目看板**：项目状态、发起方、产出文件可点击查看。
- **Token / 费用统计**：按模型、按 Agent / 本地项目的消耗明细，支持电费 / 订阅分摊 / 纯 API 三种成本口径。
- **版本与更新**：检测 OpenClaw 可用更新并支持从工作台执行升级；内置工作台更新日志、历史版本与版本回退入口。
...
- doctor、自动修复、HTTPS、远程访问和 Windows 控制仍暂时封存。
